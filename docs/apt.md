...
1. Debian and other systems that use the `apt` package manager include the upstream PostgreSQL server package `postgresql-15` by default. Percona Distribution for PostgreSQL components can only be installed together with the PostgreSQL server shipped by Percona (percona-postgresql-15). If you wish to use Percona Distribution for PostgreSQL, uninstall the `postgresql-15` package and then install the chosen components from Percona Distribution for PostgreSQL.
