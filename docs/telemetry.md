...
=== "Debian-derived distribution"
    ...
    ```{.bash data-prompt="$"}
    $ sudo PERCONA_TELEMETRY_DISABLE=1 apt install percona-ppg-server-13
    ```
...
=== "Red Hat-derived distribution"
    ...
    ```{.bash data-prompt="$"}
    $ sudo PERCONA_TELEMETRY_DISABLE=1 yum install percona-ppg-server13
    ```
...
In the first 24 hours, no information is collected nor sent.
