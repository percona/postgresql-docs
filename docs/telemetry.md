...
* The metrics from the database instance. See more in the [percona_pg_telemetry section](#perconapgtelemetry).
...
The Telemetry Subsystem extends the functionality of the database. It is extension added in Percona Distribution for PostgreSQL. The Telemetry Subsystem is enabled by default during the initial database deployment.
...
The Telemetry Subsystem creates a file in the local file system using a timestamp and a randomly generated token as the name with a `.json` extension.
...
=== "Docker"
    ...
    $ docker run -d --name psmdb --restart always \
      ...
      percona/percona-distribution-postgresql:<TAG>
    ```
    ...
    * `percona/percona/percona-distribution-postgresql:<TAG>-multi` - Specifies the image to use for the container. For example, `{{release}}-multi`. The `multi` part of the tag serves to identify the architecture (x86_64 or ARM64) and use the respective image.
...
To disable the Telemetry Subsystem, stop and drop `percona_pg_telemetry` extension. You can do this following way:
...
!!! tip
    ...
    If you wish to re-enable the Telemetry Subsystem, add the `percona_pg_telemetry` to the `shared_preload_libraries`, set `percona_pg_telemetry.enabled` to `1` and restart postgresql server.
