...
!!! note "Telemetry Agent after upgrading to 14.24 or later"
    ...
    Starting with version 14.24, `percona-pg-telemetry` no longer depends on `percona-telemetry-agent`. If your 13.x installation already has the Telemetry Agent installed, the major upgrade does **not** remove it automatically — it keeps running until you remove it. See [Telemetry Agent dependencies and removal considerations](telemetry.md#telemetry-agent-dependencies-and-removal-considerations).
